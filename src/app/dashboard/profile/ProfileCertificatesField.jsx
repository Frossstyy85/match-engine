...
export default function ProfileCertificatesField({
  profileId,
  initialValue = [],
  options = [],
  disabled = false,
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      const certificateIds = selectedNames.map((name) => nameToId.get(name)).filter(Boolean)

      await supabase.from('profile_certificates').delete().eq('profile_id', profileId)
      if (certificateIds.length > 0) {
        const { error: insertError } = await supabase
          .from('profile_certificates')
          .insert(certificateIds.map((certificate_id) => ({ profile_id: profileId, certificate_id })))
      ...
      setSaveStatus('error')
      setSaveError('Failed to save certificates.')
    } finally {
  ...
  return (
    <form onSubmit={handleSave} className='space-y-3'>
      <Field>
        <FieldLabel>Certificates</FieldLabel>
